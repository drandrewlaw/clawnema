...
async function callTrioWithRetry(streamUrl: string, condition: string, maxRetries: number = 3): Promise<{ description: string | null; usedFallback: boolean; attempts: number }> {
  ...
  const fallbackDescriptions = [
    "A breathtaking view of city skyline at night, with dazzling lights reflecting off the river.",
...
app.get('/watch', async (req: Request, res: Response) => {
  try {
    ...
    const condition = 'Describe in detail what is happening in this scene. Include visual elements, colors, lighting, movement, any people or objects visible, and overall atmosphere. What makes this scene interesting or notable?';
